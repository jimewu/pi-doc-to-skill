...
def _collect_skill_files(skill_dir: Path) -> list[Path]:
    ...
    # Fork addition: reference-mode skills keep an L1 index layer and a notes
    # layer (docs/skill-generation-spec.md), both generated by this pipeline —
    # scan them too, with the same real-directory guard as chapters/.
    for subdir in ("notes", "indexes"):
        dirpath = root / subdir
        if dirpath.exists():
            if dirpath.is_symlink() or not dirpath.is_dir():
                raise ScanError(f"{subdir} must be a real directory, not a symbolic link")
            candidates.update(_walk_markdown(dirpath))
