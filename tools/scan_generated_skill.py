...
def _collect_skill_files(skill_dir: Path) -> list[Path]:
    ...
    if notes.exists():
        if notes.is_symlink() or not notes.is_dir():
            raise ScanError("notes must be a real directory, not a symbolic link")
        candidates.update(notes.glob("*.md"))

    indexes = root / "indexes"
    if indexes.exists():
        if indexes.is_symlink() or not indexes.is_dir():
            raise ScanError("indexes must be a real directory, not a symbolic link")
        candidates.update(indexes.glob("*.md"))
